...
Note that you can also start C<toadfarm> as root to listen to port 80, but run
the workers as a non-priviledged user:
...
secondary groups. So if "some-www-user" is part of the "www" and "db" groups,
then the process will only be run as "some-www-user" part of the "www" group.

Also remember that I<without> specifying C<group> the process run with root's
current groups.
